...
// Example: Deploying a marketplace via our factory
// Our system handles the complexities of targeting the right chain.
function deployMarketplace(
    address _owner,
    string memory _name,
    uint256 _listingFee
) public {
    // Chain-specific configurations are handled by the deployment script.
    // For Arbitrum, we'd use an address resolver for key contracts.
    bazaar = new Bazaar(_owner, _name, _listingFee);
    emit MarketplaceCreated(address(bazaar), msg.sender);
}
